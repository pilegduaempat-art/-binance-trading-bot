...
from utils import init_session_state, format_currency, format_percentage
...
from notifications import send_signal_notification
from visualization import create_advanced_chart
...
def render_sidebar():
    ...
    with st.sidebar:
        ...
        with st.expander("🔑 API Settings", expanded=True):
            ...
            testnet = st.checkbox("Use Testnet", value=True, help="Use Binance Futures Testnet")
        ...
        with st.expander("🤖 Auto-Trading Settings"):
            ...
            daily_loss_limit = st.slider(
                ...
                min_value=1.0,
                max_value=20.0,
                value=st.session_state.daily_loss_limit,
                step=0.5,
            ...
            partial_tp = st.checkbox(
                ...
                help="Take profits at multiple levels (30/30/40)"
            )
        ...
        with st.expander("📊 Scan Settings"):
            ...
            num_pairs = st.slider(
                ...
                max_value=30,
                value=10,
                help="Number of top volume pairs to scan"
            )
            ...
            auto_rescan = st.checkbox(
                ...
                help="Automatically rescan at intervals"
            )
            ...
            if auto_rescan:
                rescan_interval = st.number_input(
                    ...
                    min_value=60,
                    max_value=3600,
                    value=st.session_state.get('rescan_interval', 300),
                    step=30,
                    help="Time between automatic rescans"
                )
...
def render_dashboard(config):
    ...
    if not config['api_key'] or not config['api_secret']:
        ...
        1. **Get API Keys**: Create API keys from Binance Futures
        2. **Enable Testnet**: Start with testnet for safe testing
        3. **Configure Settings**: Set your risk parameters
        4. **Start Scanning**: Click 'Scan' to find trading signals
        
        ⚠️ **Important**: Always test on testnet before using real funds!
        """)
        return
    ...
    if st.session_state.last_scan:
        time_ago = (datetime.now() - st.session_state.last_scan).total_seconds()
        st.success(
            f"✅ Last scan: {st.session_state.last_scan.strftime('%Y-%m-%d %H:%M:%S')} "
            f"({int(time_ago)}s ago)"
        )
    else:
        st.info("ℹ️ No scans performed yet. Click 'Scan' in the sidebar to start.")
    ...
    with col2:
        ...
        if st.session_state.trades_history:
            ...
            if len(recent) > 0:
                ...
                fig = go.Figure(data=[
                    go.Bar(y=pnls, marker_color=colors)
                ])
                fig.update_layout(
...
def render_signals_tab(config):
    ...
    # Handle ML training
    if config['train_ml_btn']:
        train_ml_model_interface(config)
        return
    ...
    # Display last signals
        ...
        st.info("Click 'Scan' in the sidebar to start scanning for trading signals")


def train_ml_model_interface(config):
    """Train ML model interface"""
    with st.spinner("🤖 Training ML model on BTC/USDT data..."):
        df = fetch_ohlcv(
            config['api_key'],
            config['api_secret'],
            'BTC/USDT:USDT',
        ...
        if df is None:
            st.error("Failed to fetch data for ML training")
            return
        ...
        if model:
            ...
            st.success(f"✅ Model trained successfully! Accuracy: {format_percentage(accuracy * 100)}")
            
            st.info("ML predictions will now be integrated into signal generation")
        else:
            st.error("ML training failed. Check the logs for details.")
...
def run_market_scan(config):
    ...
    with st.spinner("🔄 Scanning markets..."):
        ...
        if not exchange:
            st.error("Failed to connect to exchange. Check your API credentials.")
            return
        ...
        try:
            ...
            if not sorted_pairs:
                st.error("Failed to fetch trading pairs")
                return
...
def display_scan_results(signals_found, config):
    ...
    if not signals_found:
        st.info(
            f"⏳ No signals meeting criteria (Scan #{st.session_state.get('scan_counter', 0)}). "
            "Try adjusting confidence threshold or scanning different timeframes."
        )
        return
    
    st.success(
        f"🎯 Found {len(signals_found)} high-quality signals! "
        f"(Scan #{st.session_state.get('scan_counter', 0)})"
    )
        ...
        with col2:
            ...
            if (st.session_state.auto_trading_enabled and 
                ...
                if st.button(
                    ...
                    key=f"exec_{signal['symbol']}_{st.session_state.get('scan_counter', 0)}",
...
def display_saved_signals():
    ...
    st.info(
        ...
        f"({st.session_state.get('scan_counter', 0)} scans completed)"
    )
